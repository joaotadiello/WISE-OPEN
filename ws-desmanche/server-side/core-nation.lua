...
verifyVehicle = function(source,user_id,coord,index)
    ...
    if vehicle and placa and vname then
        if not banned then
            ...
            local vehData = nation:getVehicleData(placa)
            local dono = vehData.user
            if dono then
                ...
                -- if veh then
                    ...
                    -- end
                    if nation:isVehicleArrested(placa) or nation:isVehicleStolen(placa) then
...
finish = function(source,vehicle,vehInfo,index)
    ...
    if user_id then
        if vehInfo and index then
            ...
            DeleteEntity(NetworkGetNetworkIdFromEntity(vehInfo.vnetid))
            ...
            exports["nation-garages"]:setVehicleStolen(vehInfo.placa, false, true)
...
cancel = function(source,veh,index,vehicle)
    if veh and veh.dono and veh.vehicle and veh.model and veh.placa then
        -- garages.deleteVehicle(source,vehicle) -- Comente caso use a garagem do nation
        DeleteEntity(NetworkGetNetworkIdFromEntity(veh.vnetid))
...
ForceDeleteVehicle = function(vehicle,vnetid,prop)
    if vehicle and vnetid and prop then
        DeleteEntity(NetworkGetNetworkIdFromEntity(vnetid))
        DeleteEntity(prop)
    end
end
